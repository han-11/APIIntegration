<template>
  ...
  <tr
    :class="index % 2 === 0 ? 'bg-white rounded-lg' : 'bg-gray-100 rounded-lg'"
  >
  <td class="px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-900">
    <router-link :to="'/task/' + task.id" class="text-gray-500 ">
      #{{ task.id }}
    </router-link>
  </td>
  ...
  <td class="text-xs sm:text-sm text-gray-900 font-light px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
    <input
      ...
      class="h-4 w-4 text-sky-600 focus:ring-indigo-500 border-gray-300 rounded ml-6 lg:ml-8"
    />
  </td>
  <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm md:text-base font-light text-gray-900">

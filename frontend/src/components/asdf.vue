  ...
  <div class="flex justify-center px-4 sm:px-6 lg:px-8">
    <div class="overflow-x-auto max-w-4xl mt-5 w-full bg-white shadow-md rounded-lg">
      <div class=" mx-auto rounded-lg p-6 px-9">
        ...
        <h2 class="text-xl sm:text-2xl font-bold text-center text-sky-400 dark:text-sky-500 mb-4">
          Manage Participants
        </h2>
          ...
          <table class="min-w-full bg-white">
            ...
            <tbody>
              <tr v-for="(participant, index) in participants" :key="participant.id" :class="index % 2 === 0 ? 'bg-white border-b' : 'bg-gray-100 border-b'">
                ...
                <!-- Actions Column -->
                <td class="text-xs sm:text-sm text-gray-900 font-light px-2 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                  <button v-if="participant.isEditing" @click="saveParticipant(participant)" class="text-green-600 hover:underline">Save</button>
                  <button v-else @click="editParticipant(participant)" class="text-blue-600 hover:underline">Edit</button>
                  <button v-if="participant.isEditing" @click="cancelEdit(participant)" class="text-red-600 hover:underline ml-2">Cancel</button>
                </td>
              </tr>
            </tbody>
          </table>
